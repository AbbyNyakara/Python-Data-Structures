...
through the array. This means you should not iterate over the array more than once.
 Therefore, your solution should have a time complexity of O(n), where n is the number 
 of elements in nums.
...
def two_sum_not_optimal(nums, target):
    for i in range(len(nums)):
        num1 = nums[i]
        num2 = target - num1

        if num2 in nums[i+1::]:
            index2 = nums.index(num2, i+1)
...
print(two_sum_not_optimal([5, 1, 7, 2, 9, 3], 10))